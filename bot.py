...
async def ai_spam_loop():
    ...
    if AI_SPAM_BOT_COUNT is None:
        bot_count = len(clients)
    else:
        bot_count = min(AI_SPAM_BOT_COUNT, len(clients))
    
    if bot_count == 0:
        print("[AI SPAM] No bots available!")
        return
    ...
    print(f"[AI SPAM] Using {bot_count} bots (out of {len(clients)})")
    ...
    while AI_SPAM_ACTIVE and AI_SPAM_TARGET:
        ...
        try:
            for i in range(bot_count):
                if not AI_SPAM_ACTIVE:
                    break
                try:
                    await clients[i].send_message(AI_SPAM_TARGET, AI_SPAM_TEXT)
                    AI_SPAM_MESSAGES_SENT += 1
                    print(f"[AI SPAM] Bot {i} Sent ({AI_SPAM_MESSAGES_SENT})")
                except Exception as e:
                    print(f"[AI SPAM] Bot {i} Error: {e}")
...
async def spamtag_loop():
    ...
    # Determine bot count
    if SPAMTAG_BOT_COUNT is None:
        bot_count = len(clients)
    else:
        bot_count = min(SPAMTAG_BOT_COUNT, len(clients))
    
    if bot_count == 0:
        print("[SPAMTAG] No bots available!")
        return
    ...
    print(f"[SPAMTAG] Using {bot_count} bots (out of {len(clients)})")
    ...
    while SPAMTAG_ACTIVE and SPAMTAG_TARGET:
        try:
            ...
            # Send with all selected bots
            for i in range(bot_count):
                if not SPAMTAG_ACTIVE:
                    break
                try:
                    await clients[i].send_message(SPAMTAG_TARGET, final_text)
                    SPAMTAG_MESSAGES_SENT += 1
                    print(f"[SPAMTAG] Bot {i} Sent: {final_text[:50]}... ({SPAMTAG_MESSAGES_SENT})")
                except FloodWaitError as e:
                    print(f"[SPAMTAG] Bot {i} Flood wait: {e.seconds}s")
                    await asyncio.sleep(e.seconds)
                except Exception as e:
                    print(f"[SPAMTAG] Bot {i} Error: {e}")
...
async def handle_all_messages(event):
    ...
    if text.startswith("spamtag"):
        ...
        if parts[1].lower() == "status":
            ...
            await event.reply(
                ...
                f"Bots: {SPAMTAG_BOT_COUNT or 'All'}\n"
                ...
                if parts[i].lower() == "with" and i + 1 < len(parts):
                    ...
                    if parts[i + 1].lower() == "delay" and i + 2 < len(parts):
                        try:
                            ...
                            delay = 1.0
                        i += 2
                    elif parts[i + 1].lower() == "bot" and i + 2 < len(parts):
                        try:
                            bot_count = int(parts[i + 2])
                            if bot_count < 1:
                                bot_count = 1
                            if bot_count > len(clients):
                                bot_count = len(clients)
                        except ValueError:
                            bot_count = None
                        i += 2
                    elif parts[i + 1].isdigit():
                        # If number directly after "with"
                        try:
        ...
        bot_text = f"All {len(clients)} bots" if bot_count is None else f"{bot_count} bot(s)"
    ...
    if text.startswith("spamhere"):
        ...
        for i, part in enumerate(parts):
            ...
            if part == "with" and i + 1 < len(parts) and parts[i + 1] == "bot":
                if i + 2 < len(parts):
                    num = extract_number(parts[i + 2])
                    if num:
                        bot_count = num
        ...
        if bot_count is None:
            ...
            bot_count = min(bot_count, len(clients))
        ...
        bot_text = f"All {bot_count} bots" if bot_count == len(clients) else f"{bot_count} bot(s)"
    ...
    if text == "spamstatus":
        if AI_SPAM_ACTIVE:
            ...
            bot_text = f"All {AI_SPAM_BOT_COUNT} bots" if AI_SPAM_BOT_COUNT == len(clients) else f"{AI_SPAM_BOT_COUNT} bot(s)"
...
spamhere for [days] with [delay] delay (text) - AI Spam (ALL bots)
spamhere for [days] with [delay] delay with [count] bot (text) - AI Spam (specific bots)
stopspam - Stop AI spam
spamstatus - Check AI spam status
spamtag (@tag) with 1-60 delay - Send FOSH with tag
spamtag (@tag) with 1-60 delay with 3 bot - With specific bots
spamtag off - Stop spamtag
...
spamtag @everyone with 5 delay with 3 bot````
"""
        try:
            await client_instance.send_file(
                event.chat_id,
...
async def main():
    ...
    print("[AI] AI Spam: If no bot count specified, ALL bots will spam")
